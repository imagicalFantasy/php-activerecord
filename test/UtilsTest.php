...
class UtilsTest extends SnakeCase_PHPUnit_Framework_TestCase
{
    ...
    public function test_Utils::array_flatten()
    {
		$this->assert_equals(array(), AR\Utils::array_flatten(array()));
		$this->assert_equals(array(1), AR\Utils::array_flatten(array(1)));
		$this->assert_equals(array(1), AR\Utils::array_flatten(array(array(1))));
		$this->assert_equals(array(1, 2), AR\Utils::array_flatten(array(array(1, 2))));
		$this->assert_equals(array(1, 2), AR\Utils::array_flatten(array(array(1), 2)));
		$this->assert_equals(array(1, 2), AR\Utils::array_flatten(array(1, array(2))));
		$this->assert_equals(array(1, 2, 3), AR\Utils::array_flatten(array(1, array(2), 3)));
		$this->assert_equals(array(1, 2, 3, 4), AR\Utils::array_flatten(array(1, array(2, 3), 4)));
		$this->assert_equals(array(1, 2, 3, 4, 5, 6), AR\Utils::array_flatten(array(1, array(2, 3), 4, array(5, 6))));
	}
	...
	public function test_Utils::classify()
	{
		...
		foreach ($bad_class_names as $s)
			$class_names[] = AR\Utils::classify($s);

		$this->assert_equals($class_names, $good_class_names);
	}

	public function test_Utils::classify_singularize()
	{
		...
		$class_names = array();
		foreach ($bad_class_names as $s)
			$class_names[] = AR\Utils::classify($s, true);
	...
	public function test_Utils::wrap_strings_in_arrays()
	{
		$x = array('1',array('2'));
		$this->assert_equals(array(array('1'),array('2')),ActiveRecord\Utils::wrap_strings_in_arrays($x));

		$x = '1';
		$this->assert_equals(array(array('1')),ActiveRecord\Utils::wrap_strings_in_arrays($x));
	}
};
?>
